    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <!-- Navigation Links -->
                <div class="hidden space-x-1 lg:space-x-3 sm:-my-px sm:ms-3 lg:ms-10 sm:flex items-center">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                        <i class="fas fa-home text-xs md:text-sm" aria-hidden="true"></i>
                        <span class="hidden md:inline">{{ __('Dashboard') }}</span>
                    </x-nav-link>
                    
                    <x-nav-link :href="route('time-entries.index')" :active="request()->routeIs('time-entries.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                        <i class="fas fa-clock text-xs md:text-sm" aria-hidden="true"></i>
                        <span class="hidden md:inline">{{ __('Jornadas') }}</span>
                    </x-nav-link>

                    <x-nav-link :href="route('vacation-requests.index')" :active="request()->routeIs('vacation-requests.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                        <i class="fas fa-umbrella-beach text-xs md:text-sm" aria-hidden="true"></i>
                        <span class="hidden md:inline">{{ __('Vacaciones') }}</span>
                    </x-nav-link>

                    @if(Auth::user()->role === 'admin')
                        <x-nav-link :href="route('companies.index')" :active="request()->routeIs('companies.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-building text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Empresas') }}</span>
                        </x-nav-link>
                        <x-nav-link :href="route('users.index')" :active="request()->routeIs('users.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-users text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Usuarios') }}</span>
                        </x-nav-link>
                        <x-nav-link :href="route('reports.index')" :active="request()->routeIs('reports.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-chart-bar text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Reportes') }}</span>
                        </x-nav-link>
                        <x-nav-link :href="route('statistics.index')" :active="request()->routeIs('statistics.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-chart-line text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Estadísticas') }}</span>
                        </x-nav-link>
                        <x-nav-link :href="route('company-requests.index')" :active="request()->routeIs('company-requests.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-inbox text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Solicitudes') }}</span>
                        </x-nav-link>
                    @elseif(Auth::user()->role === 'manager')
                        <x-nav-link :href="route('companies.show', Auth::user()->company_id)" :active="request()->routeIs('companies.show')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-building text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Mi Empresa') }}</span>
                        </x-nav-link>
                        <x-nav-link :href="route('users.index')" :active="request()->routeIs('users.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-users text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Empleados') }}</span>
                        </x-nav-link>
                        <x-nav-link :href="route('reports.index')" :active="request()->routeIs('reports.*')" class="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm">
                            <i class="fas fa-chart-bar text-xs md:text-sm" aria-hidden="true"></i>
                            <span class="hidden lg:inline">{{ __('Reportes') }}</span>
